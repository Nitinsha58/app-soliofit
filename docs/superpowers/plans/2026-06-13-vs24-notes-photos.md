# VS-24 — Add Order Notes Photo Support — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Let the Add Order wizard stage **garment** photos and **measurement-notes** photos as two separate buckets, uploading each with its correct `photo_type`, and show both groups on the Review step.

**Architecture:** Frontend-only, three files. The capture UI (thumbnails + empty CTA + action sheet + in-app camera + Escape handling) currently lives once in `StepPhotos`. Extract it into a reusable `PhotoBucket` sub-component (DRY), then render **two** buckets — Garment Photos and Measurement Notes. The wizard `Draft` gains `pendingGarmentPhotos` / `pendingNotesPhotos` (replacing the single `pendingPhotos`); `handleCreate` uploads each bucket with its `photo_type`; `StepReview` renders both groups.

**Tech Stack:** Next.js client components, Tailwind with the project literal-hex palette (`#C8952A` gold, `#1A1A18`/`#6B6B67`/`#A0A09C` neutrals, `#FBF3E3` cream, `#E5E5E2` borders). No frontend test framework — verify with `tsc --noEmit` (dev Docker container) + a Playwright walkthrough (controller-run).

**Context for the implementer:**
- The data model already supports both types: backend `media/models.py` `PhotoType.GARMENT|NOTES`; the media API `uploadPhoto(orderId, file, 'garment' | 'notes')` (`lib/api/media.ts`) takes the type. **No backend/API/DB change is needed or wanted.**
- The Order Detail drawer's `PhotoSection.tsx` already shows a Garment strip + a Notes grid against the same API — reference for terminology, not to be modified.
- Photo uploads are fire-and-forget **after** order creation (installments block; media/voice don't). Keep that behavior.
- UI rules: **no emojis** anywhere — SVG icons only. Colors stay semantic (gold = brand/primary, neutrals = structure).
- z-index inside the wizard: modal `z-50` → action sheet `z-[60]` → camera `z-[70]`. Each `PhotoBucket` owns its own sheet/camera and a **capture-phase** Escape handler (so Escape closes the topmost overlay, not the whole wizard — the wizard root listens for Escape in the bubble phase at `index.tsx:59-65`).
- Density tuning across both buckets is **VS-26's** job — keep this slice functional and clean, don't over-design.

**Files:**
- Rewrite: `frontend/src/components/orders/AddOrderFlow/StepPhotos.tsx`
- Modify: `frontend/src/components/orders/AddOrderFlow/index.tsx` (`Draft`, initial state, StepPhotos props, `handleCreate` upload split)
- Modify: `frontend/src/components/orders/AddOrderFlow/StepReview.tsx` (`Draft` interface + two photo groups)

---

### Task 1: Two photo buckets end-to-end (garment + notes)

**Files:**
- Rewrite: `frontend/src/components/orders/AddOrderFlow/StepPhotos.tsx`
- Modify: `frontend/src/components/orders/AddOrderFlow/index.tsx`
- Modify: `frontend/src/components/orders/AddOrderFlow/StepReview.tsx`

- [ ] **Step 1: Rewrite `StepPhotos.tsx`** — extract `PhotoBucket`, render two sections.
...
- [ ] **Step 2: Update `index.tsx`** — split the draft bucket, wire StepPhotos, split the upload.

(a) In the `Draft` interface (lines 19–28), replace the `pendingPhotos` line:
...
(b) In the `useState<Draft>` initializer (lines 48–57), replace `pendingPhotos: [],`:
...
(c) Replace the staged-photo upload block (lines 108–113) with a split that tags each bucket:
...
(d) Replace the `step === 2` StepPhotos render (lines 173–180):
...
- [ ] **Step 3: Update `StepReview.tsx`** — interface + two photo groups.

(a) In the local `Draft` interface (lines 4–13), replace the `pendingPhotos` line:
...
(b) Replace the single Photos block (lines 114–123) with two grouped blocks:
...
```
Expected: exit 0, no output errors. (If it errors with a stale module after the rewrite, it's a type error to fix — do not restart the container; the controller handles dev-server restarts during verification.)

- [ ] **Step 5: Commit**

```bash
git add frontend/src/components/orders/AddOrderFlow/StepPhotos.tsx frontend/src/components/orders/AddOrderFlow/index.tsx frontend/src/components/orders/AddOrderFlow/StepReview.tsx
git commit -m "feat(orders): stage garment + measurement-notes photos separately in Add Order

StepPhotos renders two PhotoBucket sections (Garment Photos, Measurement
Notes), each with its own action sheet + camera. The wizard draft splits
pendingPhotos into pendingGarmentPhotos/pendingNotesPhotos; create-order
uploads each bucket with its photo_type ('garment' / 'notes'). Review
step shows the two groups separately. No backend/API change.
...
- [ ] Restart the frontend dev container (the StepPhotos rewrite + prop-signature change needs a clean bundle), then a Playwright pass (mobile 390×844 with a fake camera device + desktop 1440×900):
  - Step 2 shows **two** sections — Garment Photos and Measurement Notes — each with its own "Add photos" CTA.
  - Add a gallery photo to Garment and a camera photo (fake device shutter) to Notes; both thumbnails appear under the correct section; footer reads "Continue (2)".
  - Escape while one bucket's camera/sheet is open closes only that overlay (wizard survives).
  - Review step shows "Garment Photos" and "Measurement Notes" as separate groups.
  - Create the order against a throwaway customer **only if intended**; then confirm in Order Detail the garment photo lands in the Garment strip and the notes photo in the Notes grid (correct `photo_type`). Otherwise assert staging + Review without creating.
  - Zero console errors.
