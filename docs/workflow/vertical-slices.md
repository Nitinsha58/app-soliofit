...
- **Recently-moved feedback:** a just-dropped card gets a brief status-colored ring + an **Undo snackbar** (Undo issues the compensating reverse `/status` move; doubles as redo) that clear after ~6s. Separately, a **persistent `From <previous status>` tag** (in the *source* status's colour) stays on the card for the session — driven by a client-side `orderId → previous status` map that survives refetches.
