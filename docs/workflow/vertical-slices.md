...
- Column header shows the true total from `counts` plus the column's total order value (`value`, compact ₹) beside it.
...
- **Recently-moved highlight:** a card just dropped into a new status gets a brief light status-colored border, a `From <previous status>` tag, and an **Undo snackbar** (Undo issues the compensating reverse `/status` move; doubles as redo), all clearing after ~6s.
- **Card payment display:** the *paid* amount is colour-coded by payment state (green=paid, orange=partial, red=overdue; pending/unbilled neutral) with `/ total` neutral; the text payment badge and "₹ due" line are removed. A bottom-right last-changed stamp shows `updated_at` as 12-hour AM/PM clock time. No emoji anywhere in the UI (project rule).
