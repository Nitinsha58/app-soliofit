...
| VS-29 | Order WhatsApp messaging | Send-status WhatsApp message from Order Detail + dashboard cards + post-create modal; finalized warm template set + working-hours pickup window — decomposed into [29.1–29.8](./vs-29-whatsapp/00-overview.md) | Post-MVP — **29.1–29.8 Done (local); push 29.5–29.8 batch pending** |
...
_2026-06-14: **VS-27 (Strict installment plan) program specced — Pending.** Decomposed into five buildable sub-slices under `docs/workflow/vs-27-installments/` with a master tracker (`00-overview.md`): **27.1** backend strict billing core (atomic create + default installment, combined `PUT /orders/{id}/billing/`, removes single-row write endpoints), **27.2** legacy audit + backfill management command, **27.3** shared `QuickDateInput`, **27.4** Add Order strict plan, **27.5** drawer "Edit bill & plan" whole-plan editor. Locked invariant: `total_amount == Σ installments` and `total_amount ≥ Σ paid`, enforced on every write path. Decision recorded in **ADR-0009 (Proposed)**. 27.1 is **additive/non-breaking** (old endpoints stay live); the breaking changes (auto-default-on-omit, single-row endpoint removal, bill read-only) batch into a **cutover shipped with 27.4 + 27.5**, and the 27.2 backfill runs with that cutover — so there is no broken-UI window. The `PUT /orders/{id}/billing/` endpoint is an `OrderViewSet` detail action (not under `payments.urls`, which mounts at `…/installments/`). Execution: 27.1 → 27.3 → 27.4 + 27.5 + cutover → 27.2. **Specs only — no implementation yet; awaiting build approval per sub-slice.**_

_2026-06-19: **VS-28 (Order Detail command screen) closed — Done.** All four sub-slices shipped: 28.1 tab shell + Overview command screen, 28.2 Work tab, 28.3 Money tab, 28.4 More Details pushed screen (Customer card, Order details card, Order note tap-to-edit, Activity, DangerZone). Frontend-only program — no backend changes, no ADR. Commits on `feat/vs-28-order-detail` merged to `main`; VS-28.4 commit `838e24c`. `OrderHeader.tsx` / `QuickActions.tsx` / `OrderInfoSection.tsx` orphaned — remove in a later cleanup slice. **Next: VS-29 (Order WhatsApp messaging) — 29.1 backend then 29.2 frontend.**_
_2026-06-19: **VS-29 (Order WhatsApp messaging) closed — Done.** Initial two sub-slices: 29.1 backend (`OrderMessageLog` + `POST /orders/{id}/messages/` + detail-only `messages_sent`; commit `9400fb2`, 198/198 backend tests), 29.2 frontend (templates module + three-state `WhatsAppAction` below the Overview primary; commit `b557cd1`, type-check + browser verified on `#0022`). Click-to-chat `wa.me` deep links, optimistic send-initiated tracking, `91`+10-digit normalization (ADR-0010). **Two follow-on sub-slices added same day:** 29.3 — WhatsApp footer on dashboard cards (desktop hover overlay / mobile in-flow; `messages_sent` extended to the board action via prefetch, +1 query/page not N+1, generic list unchanged; shared `useWhatsAppSend` hook; commit `9256627`, 199/199 backend tests); 29.4 — post-create "send booked message" modal that redirects to the new order's detail drawer (commit `e6e5854`). **Deferred follow-up (tracked):** no frontend unit tests for `whatsappTemplates`/`WhatsAppAction`/`useWhatsAppSend` — the project has no frontend test framework; adding them is its own infra slice. Full WhatsApp Business API auto-send + delivery receipts stay Post-MVP (vault §2.5)._
_2026-06-19: **VS-29.5–29.8 specced — Pending.** Finalized warm "ji"-tone template set (vault §10) decomposed into four sub-slices: 29.5 vault spec (source of truth, lands at build) → 29.6 boutique working hours (additive backend) → 29.7 finalized template copy (status-aware payment context, `{item}`="order", **Partial Delivery skipped — no send action**) → 29.8 pickup window + working-hours Settings UI. 29.7 ships with a "during our working hours" fallback until 29.8 wires real hours. Documented future: Partial Delivery messaging, Delay notice, payment reminders, `action_key` non-status logging, structured item capture. **Specs only — build per sub-slice under the normal Task Plan loop.**_
_2026-06-19: **VS-29 (Order WhatsApp messaging) specced — Pending.** New post-MVP slice spun out of the Order Detail work: send a status-appropriate WhatsApp message to the customer from the Overview (just below the stage-aware primary action), and **track which status messages have been sent** so the action restyles to a "sent" state. Decomposed under `docs/workflow/vs-29-whatsapp/` — **29.1** backend (`OrderMessageLog` table + `POST /orders/{id}/messages/` + derived `messages_sent` on the order detail serializer) and **29.2** frontend (predefined templates module, `wa.me` click-to-chat with `+91` normalization, three-state Send/Sent/disabled button + Resend). Decision recorded in **ADR-0010 (Accepted)**: click-to-chat deep links (no Business API), optimistic "send-initiated" tracking (no delivery receipt), predefined client templates as interim before in-panel template management. Full WhatsApp Business API auto-send stays Post-MVP (vault §2.5). Execution: 29.1 → 29.2 (29.1 is additive/non-breaking). **Specs only — no implementation yet; awaiting build approval per sub-slice.**_
