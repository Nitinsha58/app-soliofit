...
| VS-29 | Order WhatsApp messaging | Send-status WhatsApp message from Order Detail (predefined templates + payment line); track sent-per-status, restyle button — decomposed into [29.1–29.2](./vs-29-whatsapp/00-overview.md) | Post-MVP — **Done** |
...
_2026-06-19: **VS-29 (Order WhatsApp messaging) closed — Done.** Both sub-slices shipped: 29.1 backend (`OrderMessageLog` + `POST /orders/{id}/messages/` + detail-only `messages_sent`; commit `9400fb2`, 198/198 backend tests), 29.2 frontend (templates module + three-state `WhatsAppAction` below the Overview primary; commit `b557cd1`, type-check + browser verified on `#0022`). Click-to-chat `wa.me` deep links, optimistic send-initiated tracking, `91`+10-digit normalization (ADR-0010). **Deferred follow-up (tracked):** no frontend unit tests for `whatsappTemplates`/`WhatsAppAction` — the project has no frontend test framework; adding them is its own infra slice. Full WhatsApp Business API auto-send + delivery receipts stay Post-MVP (vault §2.5)._
