...
class InstallmentListView(APIView):
    # VS-27.5 cutover — read-only. The single-row create/edit/delete endpoints were removed:
    # the schedule is now writable only via the atomic create-with-installments payload and
    # PUT /orders/{id}/billing/. GET (list) and mark-paid are the only retained paths.
    permission_classes = [IsAuthenticated]
